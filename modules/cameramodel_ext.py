...
def error(X_pix: NDArray[np.int32],
...
def rx(theta: NDArray[np.float64]) -> NDArray[np.float64]:
...
def ry(theta: NDArray[np.float64]) -> NDArray[np.float64]:
    cos_theta, sin_theta = np.cos(theta), np.sin(theta)
    zeros, ones = np.zeros_like(theta), np.ones_like(theta)
...
def rz(theta: NDArray[np.float64]) -> NDArray[np.float64]:
    cos_theta, sin_theta = np.cos(theta), np.sin(theta)
    zeros, ones = np.zeros_like(theta), np.ones_like(theta)
...
def ryxz(Theta: NDArray[np.float64]) -> NDArray[np.float64]:
    return ry(Theta[:,1]) @ rx(Theta[:,0]) @ rz(Theta[:,2])

def d_rx(theta: NDArray[np.float64]) -> NDArray[np.float64]:
    cos_theta, sin_theta = np.cos(theta), np.sin(theta)
...
def d_ry(theta: NDArray[np.float64]) -> NDArray[np.float64]:
    cos_theta, sin_theta = np.cos(theta), np.sin(theta)
    zeros = np.zeros_like(theta)
...
def d_rz(theta: NDArray[np.float64]) -> NDArray[np.float64]:
    cos_theta, sin_theta = np.cos(theta), np.sin(theta)
    zeros = np.zeros_like(theta)
...
def project(X: NDArray[np.float64],
            C: NDArray[np.float64],
            Theta: NDArray[np.float64],
            f = 1.0,
            dir_only = False
            ) -> NDArray[np.float64]:
    X_rot: NDArray[np.float64] = np.einsum('jlk,ijl->ijk', ryxz(Theta), X[:,np.newaxis,:] - C[np.newaxis,:,:])
    X_proj = np.full_like(X_rot, f)
    X_proj[:,:,:2] = f * X_rot[:,:,:2] / X_rot[:,:,2][:,:,np.newaxis]
    X_proj: NDArray[np.float64] = np.einsum('jkl,ijl->ijk', ryxz(Theta), X_proj)
    if dir_only: return X_proj
    else: return X_proj + C[np.newaxis,:,:]

def reverse_project(X_pix: NDArray[np.int32],
                    W: np.int32,
                    r: np.float64,
                    C: NDArray[np.float64],
                    Theta: NDArray[np.float64],
    ...
    X_pix_center = X_pix.astype(np.float64) + 0.5
    X_proj = np.full((*X_pix.shape[:2], 3), f, dtype=np.float64)
    X_proj[:,:,:2] = 2 * f * np.tan(phi_x / 2) * (X_pix_center[:,:,:2] / W - np.array([0.5, 0.5 / r]))
    X_proj: NDArray[np.float64] = np.einsum('jkl,ijl->ijk', ryxz(Theta), X_proj)
    if dir_only: return X_proj
    else: return X_proj + C[np.newaxis,:,:]

def transform(X: NDArray[np.float64],
              C: NDArray[np.float64],
              Theta: NDArray[np.float64],
              phi_x: np.float64,
              W: np.int32,
    ...
    X_rot: NDArray[np.float64] = np.einsum('jlk,ijl->ijk', ryxz(Theta), X[:,np.newaxis,:] - C[np.newaxis,:,:])
    X_model: NDArray[np.float64] = W / 2 * (X_rot[:,:,:2] / X_rot[:,:,2][:,:,np.newaxis] / np.tan(phi_x / 2) + np.array([1.0, 1.0 / r])[np.newaxis,np.newaxis,:])
    return X_model

def nearest_dot(X: NDArray[np.float64],
    ...
    A = V @ V.T * M
    b: NDArray[np.float64] = - np.sum(V @ X.T * M, axis=1, keepdims=True)
    lmbda = np.linalg.solve(A, b)
    X_sol = X + lmbda * V
    x_sol: NDArray[np.float64] = np.mean(X_sol, axis=0)
    return x_sol

def find_dots(X_pix: NDArray[np.int32],
              W: np.int32,
              r: np.float64,
              C: NDArray[np.float64],
              Theta: NDArray[np.float64],
...
def sgd(x: NDArray[np.float64] | np.float64,
        grad_x: NDArray[np.float64] | np.float64,
        lr: np.float64
        ) -> NDArray[np.float64] | np.float64:
    return x - lr * grad_x

def gd_adam(x: NDArray[np.float64] | np.float64,
            grad_x: NDArray[np.float64] | np.float64,
            lr: np.float64,
            s: NDArray[np.float64] | np.float64,
            r: NDArray[np.float64] | np.float64,
            t: np.float64,
            rho_1: np.float64,
            rho_2: np.float64
            ) -> NDArray[np.float64] | np.float64:
...
def fit(X_pix: NDArray[np.int32],
        W: np.int32,
        r: np.float64,
        lr: np.float64,
        max_iters: np.int32,
        X_0: NDArray[np.float64],
        C_0: NDArray[np.float64],
        Theta_0: NDArray[np.float64],
        phi_x_0: np.float64,
        X_mask: str | NDArray[np.bool_] = 'default',
        C_mask: str | NDArray[np.bool_] = 'default',
        Theta_mask: str | NDArray[np.bool_] = 'default',
        phi_x_mask = True,
        optimizer = 'SGD',
        patience = 2000,
        factor = 2
        ) -> tuple[NDArray[np.float64], NDArray[np.float64], NDArray[np.float64], np.float64, NDArray[np.float64]]:
    X_pix_center = X_pix.astype(np.float64) + 0.5
    N, K = X_pix.shape[:2]
    ...
    if ~phi_x_mask:
        tan_phi_x_2: np.float64 = np.tan(phi_x / 2)
        beta: np.float64 = W / 2 / tan_phi_x_2
    ...
    for iters in range(max_iters):      
        ...
        R = Ry @ Rx @ Rz
        X_rot: NDArray[np.float64] = np.einsum('jlk,ijl->ijk', R, X_tr)
        ...
        if C_mask != 'default': D['E', 'C'] = D['E', 'C'] * C_mask

        D['E', 'Theta'] = np.einsum('ijl,ijlk->jk', D['E', 'X_model'], D['X_model', 'Theta'])
        if Theta_mask != 'default': D['E', 'Theta'] = D['E', 'Theta'] * Theta_mask
...
def get_pixels(image: NDArray[np.float64]) -> NDArray[np.float64]:
...
def optimal_subplot_layout(n_plots: int) -> tuple[int, int]:
    cols = np.ceil(np.sqrt(n_plots))
    rows = np.ceil(n_plots / cols)
    return int(rows), int(cols)

def safe_direction(X: NDArray[np.float64],
                   id: int
                   ) -> NDArray[np.float64]:
    x_A = X[id,:]
    X_diff = x_A[np.newaxis,:] - X
    X_dir = X_diff / np.linalg.norm(X_diff)
    x_dir: NDArray[np.float64] = np.sum(X_dir, axis=0)
    return x_dir / np.linalg.norm(x_dir)

def draw_2d_views(axes: Axes,
                  X_pix: NDArray[np.int32],
                  W: np.int32,
                  H: np.int32,
                  X_model: NDArray[np.float64] = None,
                  object_corner_list=list[float] | NDArray[np.float64],
                  picture = NDArray[np.int32],
                  draw_pixels = True,
                  fontsize = 10):
    ...
    for j, ax in enumerate(axes):
        ...
        if j < X_pix.shape[1]:
            ...
            for i in range(X_pix.shape[0]):
                ...
                if X_model is None: ax.annotate(f'${i}$', xy=X_pix_center[i,j,:], va=va, ha=ha, color='black', size=fontsize, zorder=10)
                else: ax.annotate(f'${i}$', xy=X_model[i,j,:], va=va, ha=ha, color='black', size=fontsize, zorder=10)
        else: ax.axis('off')

def rotate_scene(V: NDArray[np.float64],
                 R: NDArray[np.float64]
                 ) -> NDArray[np.float64]:
    if V.ndim == 2: return V @ R.T
    elif V.ndim == 3: return np.einsum('kl,ijl->ijk', R, V)

def draw_3d_scene(ax: Axes3D,
